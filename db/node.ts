...
import type { DocumentUpdate, StorageBackend } from "./types";
...
function database() {
  ...
  databaseInstance = new DatabaseSync(path.join(root, "szeruj.sqlite"));
...
export const nodeStorage: StorageBackend = {
  ensureSchema,
  ...
  async updateDocument(id, changes: DocumentUpdate) {
    await ensureSchema();
